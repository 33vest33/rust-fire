...
namespace Oxide.Plugins
{
    ...
    public class ChatManager : RustPlugin
    {
        ...
        Dictionary<ulong, double> log = new Dictionary<ulong, double>();
        ...
        bool? OnPlayerChat(ConsoleSystem.Arg arg)
        {
            ...
            try
            {
                ...
                Interface.Oxide.RootLogger.Write(Oxide.Core.Logging.LogType.Info,
                    $"[CHAT] {sender.userID}/{sender.displayName}: {message}");
        ...
        void BroadcastChat(string name, string message, ulong userId = 0, string censormessage = "")
        {
            ...
            ConsoleNetwork.BroadcastToAllClients("chat.add", userId,
                string.IsNullOrEmpty(name) ? $"{censormessage}" : $"{name}: {censormessage}");
        }
        ...
        private void Log(LogType type, string msg)
        {
            switch (type)
            {
                case LogType.Chat:
                    Interface.Oxide.RootLogger.Write(Oxide.Core.Logging.LogType.Info,
                        $"[CHAT] {Formatter.ToPlaintext(msg)}");
                    return;
                case LogType.Mute:
                    Interface.Oxide.RootLogger.Write(Oxide.Core.Logging.LogType.Info,
                        $"[MUTE] {Formatter.ToPlaintext(msg)}");
                    return;
                case LogType.PM:
                    Interface.Oxide.RootLogger.Write(Oxide.Core.Logging.LogType.Info,
                        $"[PM] {Formatter.ToPlaintext(msg)}");
                    return;
                case LogType.ServerConsole:
                    Interface.Oxide.RootLogger.Write(Oxide.Core.Logging.LogType.Info,
                        $"[ServerConsole] {Formatter.ToPlaintext(msg)}");
                    return;
            }
        }
        ...
        void cmdChatPM(BasePlayer player, string command, string[] args)
        {
            ...
            Log(LogType.PM,
                string.Format(GetMsg("PM.LOG"), $"{player.displayName}({player.userID})",
                    $"{reciever.displayName}({reciever.userID})", message));
        }
        ...
        void cmdChatMute(BasePlayer player, string command, string[] args)
        {
            ...
            var mutePlayer =
                BasePlayer.activePlayerList.FirstOrDefault(p => p.displayName.ToLower().Contains(args[0].ToLower()));
            ...
            switch (args[0])
            {
                ...
                case "ignore":
                    if (args.Length == 2 && args[1] == "list")
                    {
                        ...
                        SendReply(player,
                            Messages["IGNORE.LIST"] + string.Join(", ",
                                playerData.BlackList.Select(p => GetPlayerData(p).Name).ToArray()));
                        return;
                    }
                ...
                case "message":
                    ...
                    if (args.Length == 1)
                    {
                        Reply(player, "CMD.CHAT.MESSAGE.HELP",
                            string.Join("\n",
                                aviableMessageColors.Select(p => $"/chat message {Format(p.Format, p.Arg)}")
                                    .ToArray()));
                        return;
                    }
        ...
        PlayerData GetPlayerData(ulong userId)
        {
            ...
            {
                if (config.Prefix != "chatmanager.default" && !PermissionService.HasPermission(userId, config.Prefix))
                    config.Prefix = "chatmanager.default";
                if (config.NameColor != "chatmanager.default" &&
                    !PermissionService.HasPermission(userId, config.NameColor))
                    config.NameColor = "chatmanager.default";
                if (config.MessageColor != "chatmanager.default" &&
                    !PermissionService.HasPermission(userId, config.MessageColor))
                    config.MessageColor = "chatmanager.default";
                return config;
            }
        ...
        void SaveData()
        {
            ...
            players_File.WriteObject(players.Where(p => !p.Value.Equals(defaultData))
        ...
        private Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {
                "NO.NAME.COLOR.CHANGED",
                "Вы не можете изменить цвет имени игрока, для начала установите один из доступных префиксов"
            },
            {"NAME.COLOR.CHANGED", "Цвет имени успешно изменен на {0}"},
            {"CMD.CHAT.MESSAGE.HELP", "ДОСТУПНЫЕ ЦВЕТА:\n{0}"},
            {
                "NO.MESSAGE.COLOR.CHANGED",
                "Вы не можете изменить цвет сообщений, для начала установите один из доступных префиксов"
            },
            {"MESSAGE.COLOR.CHANGED", "Цвет чат сообщений успешно изменен на {0}"},
            {
                "CMD.CHAT.SOUND.HELP",
                "Используйте /chat sound on или /chat sound off чтобы включить или выключить звуковое оповещение при получении ЛС"
            },
            ...
            {
                "CMD.MUTE.HELP",
                "Используйте /mute \"имя игрока\" <длительность> <причина> чтобы заблокировать чат игроку"
            },
            ...
            {
                "CMD.MUTE.ALL.HELP",
                "Используйте /muteall on или /muteall off чтобы заблокировать или разблокировать общий чат"
            },
            ...
            {
                "CMD.CHAT.CENSOR.HELP",
                "Используйте /chat censor on или /chat censor off чтобы включить или выключить показ нецензурные слов в чате"
            },
        ...
        private void LoadConfigValues()
        {
            ...
            GetConfig(ref Configuration.MuteBadWordsDefault,
                "Длительность блокировки чата за нецензурную лексику(в секундах)");
        ...
        public class ChatPrivilege
        {
            ...
            [JsonProperty("Аргумент")] public string Arg;
            [JsonProperty("Формат")] public string Format;
        }

        public class ChatConfig
        {
            [JsonProperty("Префиксы")] public List<ChatPrivilege> prefixes;
            [JsonProperty("Имена")] public List<ChatPrivilege> names;
            [JsonProperty("Сообщения")] public List<ChatPrivilege> messages;
