...
namespace Oxide.Plugins {
	[Info("InfoPanel", "Ghosst / Nogrod / Update - RustPlugin.ru", "0.9.5", ResourceId = 1356)]
	...
	public class InfoPanel : RustPlugin {
				...
				Panels = new Dictionary<string, PanelConfig> {
					{
						...
						"CargoShip", new PanelConfig {
							...
							Image = new PanelImageConfig {
								...
								Height = 0.8f,
								Margin = "0 0.15 0.1 0.1",
								Url = "https://i.imgur.com/URe0Z6b.png",
							},
							PanelSettings = new Dictionary<string, object> {
								{"InactiveColor", "1 1 1 0.1"},
								{"ActiveColor", "0.7 0.2 0.2 1"},
		...
		class PluginConfig {
			...
			public T GetPanelSettingsValue<T>(string Panel, string Setting, T defaultValue) {
				...
				if (!Panels.TryGetValue(Panel, out panelConfig)) return defaultValue;

				if (panelConfig.PanelSettings == null) return defaultValue;

				object value;
				if (!panelConfig.PanelSettings.TryGetValue(Setting, out value)) return defaultValue;
			...
			public bool CheckPanelAvailability(string Panel) {
				PanelConfig panelConfig;
				if (!Panels.TryGetValue(Panel, out panelConfig)) return false;

				if (!panelConfig.Available) return false;
		...
		protected void LoadConfigValues() {
			...
			if (UnOrderPanels.Count == 0) return;
		...
		int PanelReOrder(string DockName, string AnchorX) {
			...
			var Max = 0;
			if (SiblingPanels.Any()) Max = SiblingPanels.Max(m => m.Value.Order);

			foreach (var pPanelCfg in Settings.ThirdPartyPanels) {
				if (pPanelCfg.Value.Count == 0) {
					continue;
				}
		...
		void OnServerInitialized() {
			Clock = new Watch(Settings.GetPanelSettingsValue("Clock", "ClockUpdateFrequency (seconds)", ClockUpdateFrequency),
				Settings.CheckPanelAvailability("Clock"));

			MessageBox = new Messenger(Settings.Messages,
			...
			CompassObj = new Compass(Settings.GetPanelSettingsValue("Compass", "RefreshRate(s)", 1));

			Rad = new Radiation(Settings.GetPanelSettingsValue("Radiation", "RefreshRate(s)", 3));

			Bala = new Balance(Settings.GetPanelSettingsValue("Balance", "RefreshRate(s)", 3));

			Coord = new Coordinates(Settings.GetPanelSettingsValue("Coordinates", "RefreshRate(s)", 3));
			...
			ActiveCargoShip = UnityEngine.Object.FindObjectsOfType<CargoShip>().ToList();

			if (ActiveCargoShip.Count > 0) {
				CheckCargoShip();
			} else {
		...
		private void OnEntitySpawned(BaseEntity Entity) {
			...
			if (Entity is CargoShip && Settings.Panels["CargoShipEvent"].Available) {
				ActiveCargoShip.Add((CargoShip)Entity);
		...
		private void Unload() {
			foreach (var player in BasePlayer.activePlayerList) DestroyGUI(player);
		...
		private void LoadPanels(BasePlayer Player) {
			foreach (var Docks in Settings.Docks) {
				if (!Settings.Docks[Docks.Key].Available) continue;
			...
			foreach (var grouppedByDock in Settings.Panels.GroupBy(g => g.Value.Dock)
												   .ToDictionary(gd => gd.Key, gd => gd.Select(p => p).ToDictionary(gk => gk.Key, gk => gk.Value))) {
				if (!Settings.Docks[grouppedByDock.Key].Available) continue;

				foreach (var panelCfg in grouppedByDock.Value) {
					if (!Settings.CheckPanelAvailability(panelCfg.Key)) continue;
		...
		private void LoadPanel(IPanel Dock, string PanelName, PanelConfig PCfg) {
			...
			if (PCfg.Image != null) {
				...
				if (PCfg.Image.Color != null) Image.Color = ColorEx.Parse(PCfg.Image.Color);
			}
		}
		...
		public class Watch {
			...
			public string GetServerTime(string PlayerID, StoredData storedData) {
				return DateTime.Now.AddHours(storedData.GetPlayerPanelSettings(PlayerID, "Clock", "Offset", 0))
							   .ToString(storedData.GetPlayerPanelSettings(PlayerID, "Clock", "TimeFormat", ClockFormat), CultureInfo.InvariantCulture);
			}

			public string GetSkyTime(string PlayerID, StoredData storedData) {
				return Sky.Cycle.DateTime.ToString(storedData.GetPlayerPanelSettings(PlayerID, "Clock", "TimeFormat", ClockFormat),
					CultureInfo.InvariantCulture);
			}

			public string ShowTime(string PlayerID, StoredData storedData) {
				if (storedData.GetPlayerPanelSettings(PlayerID, "Clock", "Type", "Game") == "Server") return GetServerTime(PlayerID, storedData);
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("Clock")) return;
		...
		public class Messenger {
			...
			private void RefreshCounter() {
				...
				Counter++;
				if (Counter >= Messages.Count) Counter = 0;
			}

			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("MessageBox")) return;
		...
		private List<CargoShip> ActiveCargoShip;
		...
		public class AirplaneEvent {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("AirdropEvent")) return;
		...
		public class HelicopterEvent {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("HelicopterEvent")) return;
		...
		public class ChinukEvent {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("ChinukEvent")) return;
		...
		public class CargoShipEvent {
			...
			public void SetActivity(bool active) {
				...
				if (isActive) {
					ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("CargoShipEvent", "ActiveColor", "0.08 1 0 1"));
					return;
				}
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("CargoShipEvent")) return;
		...
		public class Radiation {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (isActive == ConVar.Server.radiation) return;

				SetActivity(ConVar.Server.radiation);

				if (!Settings.CheckPanelAvailability("Radiation")) return;
		...
		public void CheckCargoShip() {
			ActiveCargoShip.RemoveAll(p => !p.IsValid() || !p.gameObject.activeInHierarchy);
		...
		public class Balance {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("Balance")) return;
		...
		public class Coordinates {
			...
			public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels) {
				if (!Settings.CheckPanelAvailability("Coordinates")) return;
		...
		public class Compass {
			...
			public string GetDirection(string PlayerID) {
				...
				if (Settings.GetPanelSettingsValue("Compass", "TextOrAngle", "text") == "text") {
					...
					else if (PCurrent.y > 292.5 && PCurrent.y < 337.5) str = Settings.CompassDirections["nw"];
				}
		...
		public class StoredData {
			...
			public T GetPlayerSettings<T>(string PlayerID, string Key, T DefaultValue) {
				...
				if (Players.TryGetValue(PlayerID, out playerSettings)) return playerSettings.GetSetting(Key, DefaultValue);
			...
			public T GetPlayerPanelSettings<T>(BasePlayer Player, string Panel, string Key, T DefaultValue) {
				PlayerSettings playerSettings;
				if (Players.TryGetValue(Player.UserIDString, out playerSettings)) return playerSettings.GetPanelSetting(Panel, Key, DefaultValue);
			...
			public T GetPlayerPanelSettings<T>(string PlayerID, string Panel, string Key, T DefaultValue) {
				PlayerSettings playerSettings;
				if (Players.TryGetValue(PlayerID, out playerSettings)) return playerSettings.GetPanelSetting(Panel, Key, DefaultValue);
		...
		public class PlayerSettings {
			...
			public void SetPanelSetting(string Panel, string Key, string Value) {
				...
				if (!PanelSettings.TryGetValue(Panel, out settings)) PanelSettings.Add(Panel, settings = new Dictionary<string, string>());
			...
			public T GetPanelSetting<T>(string Panel, string Key, T DefaultValue) {
				...
				if (!PanelSettings.TryGetValue(Panel, out PanelConfig)) return DefaultValue;

				string value;
				if (!PanelConfig.TryGetValue(Key, out value)) return DefaultValue;
			...
			public T GetSetting<T>(string Key, T DefaultValue) {
				string value;
				if (!Settings.TryGetValue(Key, out value)) return DefaultValue;
		...
		public class IPanel {
			...
			public float GetOffset() {
				...
				var Siblings = Parent.GetChilds()
									 .Where(c => c.Value.AnchorX == AnchorX && c.Value.Order <= Order && c.Value.IsActive && c.Value.Name != Name)
									 .Select(c => c.Value)
									 .OrderBy(s => s.Order);

				foreach (var Sibling in Siblings) Offset += Sibling.Width+Sibling.Margin.y+Sibling.Margin.w;
			...
			public IPanelText AddText(string Name) {
				var Text = new IPanelText(Name, Owner, playerPanels, playerDockPanels) {
			...
			public IPanelRawImage AddImage(string Name) {
				var Image = new IPanelRawImage(Name, Owner, playerPanels, playerDockPanels) {
					ParentName = this.Name
				};
				Childs.Add(Name);
				return Image;
			...
			public IPanel AddPanel(string Name) {
				var Panel = new IPanel(Name, Owner, playerPanels, playerDockPanels) {
					ParentName = this.Name
				};
				Childs.Add(Name);
				return Panel;
			...
			public List<IPanel> GetSiblings() {
				return GetPanel(ParentName)
					   ?.GetChilds()
					   .Where(c => c.Value.AnchorX == AnchorX && c.Value.Name != Name)
					   .Select(c => c.Value)
					   .OrderBy(s => s.Order)
					   .ToList()
					?? new List<IPanel>();
			...
			public IPanel GetPanel(string PName) {
				Dictionary<string, IPanel> panels;
				IPanel panel;
				if (playerPanels.TryGetValue(Owner.UserIDString, out panels) && panels.TryGetValue(PName, out panel)) return panel;
			...
			public IPanel GetDock() {
				...
				Dictionary<string, IPanel> panels;
				IPanel panel;
				if (playerDockPanels.TryGetValue(Owner.UserIDString, out panels) && panels.TryGetValue(DockName, out panel)) return panel;
			...
			public void Hide() {
				foreach (var Panel in GetChilds().Where(p => p.Value.IsActive)) Panel.Value.Hide();
			...
			public void Reveal() {
				...
				foreach (var Child in GetChilds().Where(p => p.Value.Autoload || p.Value.IsActive).OrderBy(s => s.Value.Order)) Child.Value.Reveal();
			}

			void ReDrawPanels(List<string> PanelsName) {
				foreach (var PanelName in PanelsName) GetPanel(PanelName)?.DestroyPanel(false);

				foreach (var PanelName in PanelsName) GetPanel(PanelName)?.ShowPanel();
			}

			public void ShowPanel(bool Childs = true) {
				if (storedData.GetPlayerSettings(Owner.UserIDString, "enable", true)) {
					var Dock = GetDock();
					if (Dock != null && Dock.IsActive == false) Dock.ShowPanel(false);

					var ActivePanelsAfterThis = GetActiveAfterThis();

					foreach (var PanelName in ActivePanelsAfterThis) GetPanel(PanelName)?.DestroyPanel(false);
					...
					if (Childs) {
						foreach (var Child in GetChilds().Where(p => p.Value.Autoload || p.Value.IsActive).OrderBy(s => s.Value.Order)) Child.Value.ShowPanel();
					}

					foreach (var PanelName in ActivePanelsAfterThis) GetPanel(PanelName)?.ShowPanel();
				} else {
					ShowPanelIfHidden();
				}
			}
			...
			public void DestroyPanel(bool Redraw = true) {
				foreach (var Panel in GetChilds().Where(p => p.Value.IsActive)) Panel.Value.DestroyPanel(false);
				...
				if (Redraw) ReDrawPanels(GetActiveAfterThis());
			...
			public void Remover() {
				foreach (var Child in GetChilds()) Child.Value.Remover();
		...
		public class IPanelText : IPanel {
			...
			public IPanelText(string Name,
							  BasePlayer Player,
							  Dictionary<string, Dictionary<string, IPanel>> playerPanels,
							  Dictionary<string, Dictionary<string, IPanel>> playerDockPanels) : base(Name, Player, playerPanels, playerDockPanels) {
		...
		public class IPanelRawImage : IPanel {
			...
			public IPanelRawImage(string Name,
								  BasePlayer Player,
								  Dictionary<string, Dictionary<string, IPanel>> playerPanels,
								  Dictionary<string, Dictionary<string, IPanel>> playerDockPanels) : base(Name, Player, playerPanels, playerDockPanels) {
		...
		private void InitializeGUI(BasePlayer player) {
			if (!storedData.GetPlayerSettings(player.UserIDString, "enable", true)) return;
			...
			foreach (var Dock in PlayerDockPanels[player.UserIDString]) {
				if (Dock.Value.Childs.Count != 0) Dock.Value.ShowPanel();
			}
		}
		...
		private bool PanelRegister(string PluginName, string PanelName, string json) {
			...
			if (LoadedPluginPanels.TryGetValue(PluginName, out loadedPlugin) && loadedPlugin.Contains(PanelName)) return true;
			...
			foreach (var Docks in PlayerDockPanels) {
				if (Docks.Value.ContainsKey(Cfg.Dock)) LoadPanel(Docks.Value[Cfg.Dock], PanelName, Cfg);
			}

			if (!LoadedPluginPanels.TryGetValue(PluginName, out loadedPlugin)) LoadedPluginPanels.Add(PluginName, loadedPlugin = new List<string>());
		...
		private bool ShowPanel(string PluginName, string PanelName, string PlayerId = null) {
			if (!Settings.ThirdPartyPanels[PluginName][PanelName].Available) return false;
			...
			foreach (var PlayerID in PlayerPanels.Keys) PlayerPanels[PlayerID][PanelName].ShowPanel();
		...
		private bool HidePanel(string PluginName, string PanelName, string PlayerId = null) {
			...
			foreach (var PlayerID in PlayerPanels.Keys) PlayerPanels[PlayerID][PanelName].DestroyPanel();
		...
		internal static Vector4 Vector4Parser(string p) {
			...
			if (strArrays.Length != 4) return Vector4.zero;
