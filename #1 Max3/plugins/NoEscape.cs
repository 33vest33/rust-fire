...
namespace Oxide.Plugins
{
    ...
    class NoEscape : RustPlugin
    {
        ...
        private void LoadDefaultConfig()
        {
            ...
            GetConfig("Основное", "Поддержка плагина GuiAnnouncements", ref useGuiAnnouncement);
			...
			GetConfig("Блокировка", "Блокировать установку штурмовых лесниц в рейд блоке", ref canDuel);
            SaveConfig();
        }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info, Item item)
        {
            ...
            if (clansAPI && Clans.Call("GetClanOf", entity.OwnerID) != null && !IsClanMember(entity.OwnerID, player.userID))
            {
                ...
                foreach (var uid in team.ToList())
                {
                    if (!raid.owners.Contains(uid))
                    {
                        ...
                        if (useGuiAnnouncement)
                        {
                            if (plugins.Exists("GUIAnnouncements"))
                            {
                                if (p)
                                    GUIAnnouncements?.Call("CreateAnnouncement", Messages["yourbuildingdestroy"], "Red",
                                        "white", p);
                            }
                        }
                        raid.owners.Add(uid);
            ...
            else
            {
                if (!raid.owners.Contains(entity.OwnerID))
                {
                    ...
                    if (useGuiAnnouncement)
                    {
                        ...
                        {
                            if (p != null)
                                GUIAnnouncements?.Call("CreateAnnouncement", Messages["yourbuildingdestroyOwner"], "Red",
                                            "White", p, player.displayName);
                        }
                    }
                    if (blockOwner) { raid.owners.Add(entity.OwnerID); }
                    if (useDamageScale)
                    {
                        if (justCreated && p)
                        {
        ...
        void BlockPlayer(BasePlayer player, string mode = "", bool owner = false)
        {
            ...
            if (mode == "raid")
            {
                if (!timers.ContainsKey(player.userID))
                {
                    player.ChatMessage(string.Format(Messages[owner ? "ownerhome" : "blockactive"], FormatTime(TimeSpan.FromSeconds(owner ? ownerBlockTime : blockTime))));
                }
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info, BaseCombatEntity victim)
        {
            ...
            if (entity is BasePlayer && info.Initiator is BasePlayer)
            {
                if (blockAttack)
                {
                    if (!IsDuelPlayer(initiator) && entity != null)
                    {
                        GetAroundPlayers(entity.GetNetworkPosition()).ForEach(p => BlockPlayer(p, "attack"));
                    }
                }
            }
            ...
            if (useDamageScale)
            {
                ...
                if (clansAPI && Clans.Call("GetClanOf", entity.OwnerID) != null)
                {
                    var team = (List<ulong>)Clans.Call("GetClanMembers", entity.OwnerID);
		...
		object TPPlayer(BasePlayer player)
        {
            if (player == null) return null;
            if (canDuel)
            {
                ...
                if (seconds > 0)
                {
                    SendReply(player, string.Format(Messages["blockDuel"], seconds));
                    return false;
                }
            }
            return null;
        ...
        void SendOfflineMessage(ulong id, string raidername)
        {
            ...
            if (LastNotice == null)
            {
                ...
                VKBot?.Call("VKAPIMsg", text, userVK);
            ...
            else
            {
                if (DateTime.TryParseExact(LastNotice, "dd.MM.yyyy HH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out LNT))
                {
                    if (TimeLeft(LNT).TotalMinutes >= offlineOut)
                    {
                        string text = TextReplace(formatMessage,
                            new KeyValuePair<string, string>("attacker", raidername));
                        VKBot?.Call("VKAPIMsg", text, null, userVK);
                        string LastRaidNotice = DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss");
                        VKBot?.Call("VKAPISaveLastNotice", id, LastRaidNotice);
                    }
                }
        ...
        {
            return timers.ContainsKey(target.userID);
        }
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"blockactive", "Строение разрушено, активирован рейд блок на <color=#ECBE13>{0}</color>\nНекоторые функции временно недоступны."},
			...
			{ "blockDuel", "Вы не можете использовать Duel во время рейда, подождите {0} сек." },
            ...
            { "yourbuildingdestroyOwner", "Внимание! Ваш дом рейдит игрок {0}! Добавлена отметка на карту" },
