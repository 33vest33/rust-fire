...
namespace Oxide.Plugins
{
    ...
    class SkipNightVote : CovalencePlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            VoteOpen = false;
            TimeRemaining = 0;
            TimeRemMSG = GetMSG("timeRem").Replace("{secCol}", configData.Messaging.MSGColor)
                .Replace("{mainCol}", configData.Messaging.MainColor);
        ...
        private void OpenVote()
        {
            ...
            VoteOpen = true;
            var msg = GetMSG("voteMSG").Replace("{secCol}", configData.Messaging.MSGColor)
                .Replace("{mainCol}", configData.Messaging.MainColor).Replace("{reqVote}",
                    (configData.Options.RequiredVotePercentage * 100).ToString());
        ...
        {
            server.Broadcast(
                $"{configData.Messaging.MainColor}{ReceivedVotes.Count} / {RequiredVotes}</color> {configData.Messaging.MSGColor}{GetMSG("have voted to skip night")}</color>");
            CountTimer = timer.In(configData.Messaging.DisplayCountEvery, ShowCountTimer);
        }
        ...
        private void CheckTime()
        {
            if (!VoteOpen)
            {
                if ((server.Time.TimeOfDay >= TimeSpan.Parse(configData.Options.TimeToOpen) &&
                     server.Time.TimeOfDay < TimeSpan.Parse("23:59:59")) ||
                    (server.Time.TimeOfDay >= TimeSpan.Parse("00:00:00") &&
                     server.Time.TimeOfDay < TimeSpan.Parse(configData.Options.TimeToSet)))
                {
                    OpenVote();
                }
                else
            ...
            {
                if (server.Time.TimeOfDay >= TimeSpan.Parse(configData.Options.TimeToSet) &&
                    server.Time.TimeOfDay < TimeSpan.Parse(configData.Options.TimeToOpen))
                {
                    VoteEnd(false);
                }
            }
        ...
        private void VoteEnd(bool success)
        {
            ...
            if (success)
            {
                ...
                server.Broadcast(
                    $"{configData.Messaging.MainColor}{GetMSG("Voting was successful, skipping night.")}</color>");
            }
            else
            {
        ...
        private void cmdVoteDay(IPlayer player, string command, string[] args)
        {
            if (VoteOpen)
            {
                if (!AlreadyVoted(player.Id))
                {
                    ...
                    if (DisplayCountEveryVote)
                        server.Broadcast(
                            $"{configData.Messaging.MainColor}{ReceivedVotes.Count} / {RequiredVotes}</color> {configData.Messaging.MSGColor}{GetMSG("have voted to skip night", player.Id)}</color>");
                    if (ReceivedVotes.Count >= RequiredVotes)
                        VoteEnd(true);
                    return;
                }
                ...
                player.Reply(
                    $"{configData.Messaging.MainColor}{GetMSG("There is not a vote currently open", player.Id)}</color>");
        }
        ...
        private void cmdAdminVote(IPlayer player, string command, string[] args)
        {
            ...
            {
                player.Reply(
                    $"{configData.Messaging.MainColor}/nightvote open</color> {configData.Messaging.MSGColor}- {GetMSG("Force open a new vote", player.Id)}</color>");
                player.Reply(
                    $"{configData.Messaging.MainColor}/nightvote close</color> {configData.Messaging.MSGColor}- {GetMSG("Cancel the current vote", player.Id)}</color>");
                return;
            }
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            {
                "voteMSG",
                "{secCol}Type</color> {mainCol}/voteday</color> {secCol}now if you want to skip night. If </color>{mainCol}{reqVote}%</color> {secCol}of players vote night will be skipped</color>"
            },
            {
                "timeRem",
                "{secCol}Voting ends in</color> {mainCol}{time} {type}</color>{secCol}, use </color>{mainCol}/voteday</color>{secCol} to cast your vote</color>"
            }
        };
