...
namespace Oxide.Plugins
{
    ...
    class GameStoresRUST : RustPlugin
    {
		...
		private string Request2 => $"https://gamestores.ru/api/?shop_id={Config["SHOP.ID"]}&secret={Config["SECRET.KEY"]}&server={Config["SERVER2.ID"]}";
        ...
        private Dictionary<string, long> RequestDelay = new Dictionary<string, long>();
        private Dictionary<string, long> OpenBucketDelay = new Dictionary<string, long>();
                        ...
                        switch (System.Convert.ToInt32(Response["code"]))
                        {
                            case 100:
                                ...
                                if (System.Convert.ToBoolean(Config["UI.ENABLED"]))
                                {
                                    ...
                                    CuiHelper.AddUi(Player, UI);
                                    RequestDelay.Remove(Player.UserIDString);
                                    return;
                                }
        ...
        private void SendRequest(Dictionary<string, string> Args, BasePlayer Player = null, bool exec = true)
        {
            ...
            if (HasDelay(Player, true) == true)
                return;

            RequestDelay.Add(Player.UserIDString, GetTimestampNow());
            OpenBucketDelay.Add(Player.UserIDString, GetTimestampNow());
        ...
        private void cmdUiBack(ConsoleSystem.Arg Args)
        {
            ...
            if (Index[Player] <= 14)
            {
                Player.ChatMessage($"Это первая страница корзины.");
                return;
            }

            if (HasDelay(Player) == true)
                return;
            ...
            webrequest.EnqueueGet($"{Request}&items=true&steam_id={Player.UserIDString}", (code, response) =>
            {
                switch (code)
                {
                    ...
                    case 200:
                        ...
                        {
                            Player.ChatMessage($"Это последняя страница корзины.");
                            RequestDelay.Remove(Player.UserIDString);
                            return;
                        }
                        else
        ...
        #region[Helper] HasDelay
        private bool HasDelay(BasePlayer Player, bool OpenBucket = false)
        {
            if (OpenBucket)
            {
                if (OpenBucketDelay.ContainsKey(Player.UserIDString))
                {
                    if (OpenBucketDelay[Player.UserIDString] + 3 <= GetTimestampNow())
                    {
                        OpenBucketDelay.Remove(Player.UserIDString);
                    }
                    else
                    {
                        Player.ChatMessage($"Корзину можно открыть раз в 3 секунды.");
                        return true;
                    }
                }
            }

            if (RequestDelay.ContainsKey(Player.UserIDString))
            {
                if (RequestDelay[Player.UserIDString] + 30 <= GetTimestampNow())
                {
                    RequestDelay.Remove(Player.UserIDString);
                    return false;
                }

                if (RequestDelay[Player.UserIDString] + 5 <= GetTimestampNow())
                {
                    Player.ChatMessage($"Предыдущий запрос был завершен некорректно. Попробуйте еще раз через {Math.Abs(RequestDelay[Player.UserIDString] + 30 - GetTimestampNow())} сек.");
                }
                else
                {
                    Player.ChatMessage($"Дождитесь завершения предыдущего запроса");
        ...
        private void OnPlayerDisconnected(BasePlayer player)
        {
            if (System.Convert.ToBoolean(Config["TOP.USERS"]))
            {
                if (Config["SERVER2.ID"].ToString() == "0")
                {
                    Debug.LogWarning("Need set SERVER2.ID in configurations to send info for top players");
                }
                else
                {
                    Dictionary<string, object> args = new Dictionary<string, object>();
                ...
                else
                {
                    for (int i = 0; i < (int)(Stats.Count / 20) + 1; i++)
                    {
                        if (Stats.Count > 0)
                        {
                            ...
                            string request2 = $"{Request2}&json=true&data={JsonConvert.SerializeObject(Temp)}";

                            //Debug.LogWarning(request2);
                            webrequest.EnqueueGet(request2, (code, res) =>
                            {
                                switch (code)
                                {
                                    case 0:
                                        Debug.LogError("Api does not responded to a request2");
                                        break;
                                    case 200:
                                        break;
                                    case 404:
                ...
                else
                {
                    for (int i = 0; i < (int)(Leaves.Count / 20) + 1; i++)
                    {
                        if (Leaves.Count > 0)
                        {
                            ...
                            string request2 = $"{Request2}&action=leaves&type=json&data={JsonConvert.SerializeObject(Temp)}";
