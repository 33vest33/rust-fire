...
namespace Oxide.Plugins
{
    ...
    class CheckPlayers : RustPlugin
    {
        ...
        private void SendSkype(BasePlayer player, string cmd, string[] args)
        {
            ...
            string text = string.Format(GetMsg("КомандаСкайп", player), player.displayName, player.UserIDString, contact);
            if (CheckCMDPlayer == player)
            {
                PrintWarning(text);
        ...
        private void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                {"Сообщение модератору", "[CheckPlayers] Игрок {0} (steamcommunity.com/profiles/{1}/) предоставил скайп для проверки: {2}"}
            }, this);
        }
