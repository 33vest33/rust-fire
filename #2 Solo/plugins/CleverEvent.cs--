...
namespace Oxide.Plugins
{
    ...
    public class CleverEvent : RustPlugin
    {
        ...
        private List<Question> questionList = new List<Question>
        {
            new Question
            {
                Text = "Сколько нужно градусов, чтобы закипела вода?",
            ...
            new Question
            {
                Text = "Что крафтится с помощью ноутбука,камеры и 40 МВК",
                Answers = new Dictionary<string, bool>
                {
                    ["Огненная турель"] = false,
                    ["Лазерный прицел"] = false,
                    ["Автоматическая турель"] = true
                }
            },
            new Question
            {
                Text = "Где восходит солнце? ",
                Answers = new Dictionary<string, bool>
                {
                    ["На западе"] = false,
                    ["На восходе"] = false,
            ...
            new Question
            {
                Text = "Что Винни-Пух считал лучшим подарком? ",
                Answers = new Dictionary<string, bool>
                {
                    ["Сахар"] = false,
                    ["Мёд"] = true,
                    ["Ход-Дог в лаваше(двойной)"] = false
                }
            },
            new Question
            {
                Text = "Какой предмет используется для изучения вещей?",
                Answers = new Dictionary<string, bool>
                {
                    ["Фрагменты Металла"] = false,
                    ["Металлолом"] = true,
                    ["Метал высокого качества"] = false
                }
            },
            new Question
            {
                Text = "Москву называют третьим … ",
                Answers = new Dictionary<string, bool>
                {
                    ["Миром"] = false,
                    ["По списку"] = false,
            ...
            new Question
            {
                Text = "Чем питается китовая акула?",
                Answers = new Dictionary<string, bool>
                {
                    ["Китами"] = false,
                    ["Планктоном"] = true,
            ...
            new Question
            {
                Text = "В каких единицах измерения измеряют солёность морской воды? ",
                Answers = new Dictionary<string, bool>
                {
                    ["В промиллях"] = true,
                    ["В литрах"] = false,
                    ["В миллиметрах ртутного столба"] = false
                }
            },
            new Question
            {
                Text = "Какими словами традиционно начинаются русские народные сказки?",
                Answers = new Dictionary<string, bool>
                {
                    ["Трали-вали"] = false,
                    ["Жили-были"] = true,
            ...
            new Question
            {
                Text = "Какое животное первым попало на орбиту земли?",
                Answers = new Dictionary<string, bool>
                {
                    ["Человек"] = false,
                    ["Кот"] = false,
            ...
            new Question
            {
                Text = "Чему равна окружность земного шара?",
                Answers = new Dictionary<string, bool>
                {
                    ["63 581 км"] = false,
                    ["15 000 км"] = false,
        ...
        {
			ImageLibrary.Call("AddImage", "https://i.imgur.com/yTUISEb.png", "iPhone_Clever");
        ...
        private string PrepareGame(int questionAmount, int finishPrize)
        {
            ...
            foreach (var check in BasePlayer.activePlayerList)
                ReplyWithHelper(check, "Начинается регистрация на игру \"Клевер\", в ней вы можете получить деньги на баланс магазина, отвечая на вопросы.\n  <color=#F788BA>Зарегистрироваться</color>: /clever");
        ...
        private void SwitchQuestion()
        {
            ...
            if (currentGame.Questions.Count == currentGame.CurrentQuestionIndex)
            {
                ...
                {
                    ReplyWithHelper(check.Key, $"Вы, и <color=#F788BA>{currentGame.PlayerAnswers.Last().Count(p => p.Value)}</color> чел. получили по <color=#F788BA>{eachPrize}</color> рублей на баланс! Для перевода денег в магазин, нажмите на баланс");
                ...
                foreach (var check in BasePlayer.activePlayerList.Where(p => !currentGame.PlayerAnswers.Last().ContainsKey(p)))
                    ReplyWithHelper(check, $"Игра закончена, победители получили по <color=#F788BA>{eachPrize}</color> рублей! Для перевода денег в магазин, нажмите на баланс");
            ...
            foreach (var check in currentGame.PlayerAnswers[currentGame.CurrentQuestionIndex - 1])
            {
                if (!currentGame.PlayerAnswers[currentGame.CurrentQuestionIndex].ContainsKey(check.Key) && check.Value)
                {
                    if (check.Key != null && check.Key.IsConnected)
                        ReplyWithHelper(check.Key, "Вы не ответили на вопрос, вы <color=#FF5733>проиграли</color>");
                }
        ...
        {
            foreach (var check in BasePlayer.activePlayerList)
                ReplyWithHelper(check, $"Продолжается регистрация на игру \"Клевер\", в ней вы можете получить деньги на баланс магазина, отвечая на вопросы.\n  <color=#F788BA>Зарегистрироваться</color>: /clever\nОсталось: <color=#F788BA>{currentGame.LeftTimeString()}</color>");
        ...
        private void cmdChatClever(BasePlayer player)
        {
            ...
            if (currentGame != null)
            {
                ...
                {
                    ReplyWithHelper(player, "Вы успешно зарегистрировались на игру! Закрыть телефон вы можете нажав на <color=#F788BA>крестик</color>");
        ...
        private void cmdConsoleHandler(ConsoleSystem.Arg args)
        {
            ...
            if (args.Args[0].ToLower() == "close")
            {
                ...
                container.Add(new CuiButton
                {
                    RectTransform = { AnchorMin = "0.0099996 0.8835207", AnchorMax = "0.9940634 0.9694132", OffsetMax = "0 0" },
                    Button = { Close = MenuLayer, Color = "0 0 0 0", Command = "UI_Clever acceptclose" },
                    Text = { Text = "" }
                }, MenuLayer, MenuLayer + ".Close");
                ReplyWithHelper(player, "Для того чтобы закрыть телефон, нажмите на <color=#F788BA>крестик</color> в правом верхнем углу телефона!");
                CuiHelper.AddUi(player, container);
            }
                        ...
                        StoreHandler.Call("AddMoney", player.userID, (float) playerBalance[player.userID].Balance,
                            "Клевер: вывод средств", (Action<bool>) ((a) =>
                            {
                                ...
                                else
                                {
                                    ...
                                    ReplyWithHelper(player, $"Вы успешно вывели: <color=#F788BA>{playerBalance[player.userID].Balance} рублей.</color>");
            ...
            if (args.Args[0].ToLower() == "answer")
            {
                ...
                ReplyWithHelper(player, $"Вы успешно ответили на вопрос!\n<color=#F788BA>Ответ</color>: {currentGame.CurrentQuestion().Answers.ElementAt(answer).Key}");
            }
        }
