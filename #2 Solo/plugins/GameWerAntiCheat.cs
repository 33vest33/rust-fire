...
namespace Oxide.Plugins
{
    ...
    public class GameWerAntiCheat : CovalencePlugin
    {
        ...
        private List<string> ListAuthedPlayers { get; } = new List<string>();
        ...
        void Unload()
        {
            List<string> listCachePlayers= new List<string>(this.ListAuthedPlayers);
            for (var i = 0; i < listCachePlayers.Count; i++)
                this.RunUnAuthPlayer(this.players.FindPlayerById(listCachePlayers[i]));
            ListAuthedPlayers.Clear();
            GameWer.SDK.Interface.Shutdown();
        }
        
        private void OnUserConnected(IPlayer player)
        {
            this.LogWarning("[DEBUG]: OnUserConnected - " + player.Id);
            ulong steamid;
        ...
        {
            this.LogWarning("[DEBUG]: OnUserDisconnected " + player.Id);
            ulong steamid;
            if (ulong.TryParse(player.Id, out steamid))
            {
        ...
        void OnNetworkStatusChange(bool status) {
            ...
            if(status == true)
            {
				...
				} else
                {
                    ...
                    licensKey = GameWer.SDK.Interface.Settings.Licenskey;
					string temporaryKey = (string)GameWer.SDK.Interface.Settings.TemporaryKey;
					licensKey = (string.IsNullOrEmpty(temporaryKey) ? licensKey : temporaryKey);
				}
            ...
            {
                List<string> listAuthedPlayersCache = new List<string>(this.ListAuthedPlayers);
                for (var i = 0; i < listAuthedPlayersCache.Count; i++)
                    this.RunUnAuthPlayer(this.players.FindPlayerById(listAuthedPlayersCache[i]));
            }
        }
		
        void OnNeedKickPlayer(ulong steamid, string reason) {
            this.LogWarning("[DEBUG]: OnNeedKickPlayer - " + steamid + " - " + reason);
        ...
        {
            byte[] token = ParseAuthToken(player);
            int appid = GameWer.SDK.Interface.GetAppID(token);
            this.LogWarning("[DEBUG]: RunAuthPlayer - " + player.Id + " -" + appid);
            if (appid != 480 && SettingsNeedGameWerFromLicens == false)
                return;
            
            if (this.HasNetworkStatus == true)
            {
                if (this.ListAuthedPlayers.Contains(player.Id))
                    this.RunUnAuthPlayer(this.players.FindPlayerById(player.Id));
                this.LogWarning("[DEBUG]: Player not authed - " + player.Id + " - start auth!");
                Interface.Oxide.CallHook("OnGameWerAuthPlayer", ulong.Parse(player.Id));
                this.ListAuthedPlayers.Add(player.Id);
                GameWer.SDK.Interface.AuthPlayer(ulong.Parse(player.Id), (SettingsCheckVPN ? player.Address.Split(':')[0] : "0.0.0.0"));
            } else
                this.LogWarning("[DEBUG]: RunAuthPlayer:2 - " + player.Id + " - network false");
                
        }

        void RunUnAuthPlayer(IPlayer player)
        {
            this.LogWarning("[DEBUG]: RunUnAuthPlayer - " + player.Id);
            if (this.ListAuthedPlayers.Contains(player.Id))
            {
                this.LogWarning("[DEBUG]: RunUnAuthPlayer:2 - " + player.Id + " - player authed, start unauth!");
                Interface.Oxide.CallHook("OnGameWerUnAuthPlayer", ulong.Parse(player.Id));
                this.ListAuthedPlayers.Remove(player.Id);
                if (this.HasNetworkStatus == true)
                    GameWer.SDK.Interface.Logout(ulong.Parse(player.Id));
                else
                    this.LogWarning("[DEBUG]: RunUnAuthPlayer:2 - " + player.Id + " - player authed, network false!");
            }
        }

        void OnTickRequestScreenshot(IPlayer player, bool repeat = false)
        {
            if (this.HasNetworkStatus == true && this.ListAuthedPlayers.Contains(player.Id))
